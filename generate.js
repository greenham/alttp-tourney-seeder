...
for (let h = 0; h < genNumGroups; h++) {
	...
	let groupNamesCopy = JSON.parse(JSON.stringify(groupNames));
	generateGroups(bucketsCopy, numGroups, groupSize, groupNamesCopy);
}
...
function generateGroups(buckets, numGroups, groupSize, groupNames)
{
	...
	for (let i = 0; i < numGroups; i++) {
		...
		newGroup.name = groupNames.splice(Math.floor(Math.random() * groupNames.length), 1)[0];
		newGroup.members = [];
